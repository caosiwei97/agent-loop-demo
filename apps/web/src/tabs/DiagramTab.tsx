...
function waitForMermaid(): Promise<void> {
  ...
  const existing = document.querySelector('script[data-mermaid-loaded]');

  if (!existing) {
    return new Promise<void>((resolve, reject) => {
      const script = document.createElement('script');
      script.src = '/vendor/mermaid.js';
      script.setAttribute('data-mermaid-loaded', 'true');
      script.onload = () => {
...
export default function DiagramTab({ caseId }: DiagramTabProps) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const code = await fetchFile(`cases/${caseId}/diagram.mmd`);
        if (cancelled) return;
        ...
        let result;
        try {
          result = await window.mermaid.render(svgId, code.trim());
        } catch (renderErr) {
          // mermaid appends an error SVG container (id="d"+svgId) to document.body on failure.
          // Clean it up so it doesn't leak outside this tab.
          const errorEl = document.getElementById('d' + svgId);
          if (errorEl) errorEl.remove();
          throw renderErr;
        }
        // Also clean up the wrapper div mermaid creates (id="d"+svgId) on success
