...
把三根保险丝和七种退出路径串在一起，就是完整的 Agent Loop。三根保险丝在循环的不同阶段分别守护不同风险，谁也不碍谁，但加在一起就是一张网。
...
- 保险丝 1（死循环检测）在工具执行后检查——调用指纹 + 结果指纹双重判定
- 保险丝 2（Token 预算）在每轮结束后检查——90% nudge + 递减回报检测
- 保险丝 3（截断恢复）在检测到 `finishReason === 'length'` 时触发——渐进式恢复
...
核心结构是 `for (turn <= MAX_TURNS)` 循环，每轮依次检查三根保险丝，任何一根触发都会设置 `stopped = true`。

## 延伸思考

- `max_turns` 设多少合适？太少可能完不成任务，太多浪费 Token。根据任务复杂度怎么设？
- 这些容错和保险丝机制，应该由框架来做还是开发者在业务层做？各自的优势和风险是什么？
- 今天的"保险丝"都是反应式的——出了问题才处理。有没有可能做到预防式的？
