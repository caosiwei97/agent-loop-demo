...
用 Claude Code 做一次全局替换——比如把 moment 换成 dayjs——整个过程异常丝滑。文字一个字一个字往外蹦，紧接着就是工具调用，读文件、改文件、读下一个文件，几乎感觉不到等待。

停下来想一下：模型还在生成第二个工具调用的参数时，第一个工具就已经跑完返回了。这不是模型变快了，是"边生成边执行"的调度在起作用。

回到经典的 ReAct 模式，同样的任务长这样：
...
每一步都在等——等模型想完、等工具返回。串行跑下来，光等待就能吃掉 20 秒。流式架构在串行链条的每一段里都插入了并行，把等待压到最小。

今天拆三个工程问题——也是生产级 Agent 和 demo 级 Agent 之间最大的鸿沟：
